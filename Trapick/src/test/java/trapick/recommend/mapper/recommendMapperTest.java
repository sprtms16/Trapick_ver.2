...
public class recommendMapperTest {
	...
	@Test
	public void test(){
		List<Integer> feeder = mapper.findFeeder(2);
		...
		for(int i=0; i<feeder.size(); i++){
			Temp = mapper.findFeed(feeder.get(i));
		...
		for(int i=0; i<feed.size();i++){
			Temp = mapper.findSchedule(feed.get(i));
			for(int j=0; j<Temp.size();j++){
		...
		for(int i=0; i<schd_idx.size();i++){
			Temp = mapper.findLandmark(schd_idx.get(i));
			for(int j=0; j<Temp.size();j++){
